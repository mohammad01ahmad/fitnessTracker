...
- `makeWASocket` explicitly sets `getMessage` (a stub — no message store is kept, so resend requests can't be fulfilled; see the `ponytail:` comment in `socket.ts` for the upgrade path) and `markOnlineOnConnect: false` (default `true` suppresses phone push notifications, including for this bot's own confirmations).

**Two things `docs/baileys-production-audit-2.md` recommends that are deliberately *not* in the code — don't re-add them from that doc without reading this first:**

- **No "connected but wedged" watchdog.** One was added (§1 Fix 4 of the audit) and removed on 2026-07-31 after it killed a demonstrably healthy connection — the logs showed `got my own devices` (an inbound server-pushed notification, requiring a genuinely live two-way socket) minutes before a timer fired and restarted the container anyway. Root cause: it reset only on `'open'`, never on any liveness signal after that, so it fired ~15min after *every* successful connect regardless of health — not an edge case, the steady-state behavior. It was also solving a problem Baileys already solves: `Socket/socket.js` pings every 30s and self-closes with `DisconnectReason.connectionLost` (408) after 35s of silence, which isn't in any of `reconnect.ts`'s terminal sets and so already flows through the normal backoff path. A frozen event loop (the one scenario an app-level timer could theoretically add value for) would also stop the watchdog's own timer from firing, so there's no real gap left for a timer-based check to cover.
- **No `shouldSyncHistoryMessage: () => false`.** Also added per the audit (§6) and also removed 2026-07-31. Baileys' own `makeSocket` logs an unconditional `⚠️ DANGER` warning whenever every history-sync type is disabled, because the initial history sync is one of the paths that populates LID mappings — disabling it entirely undermines the LID handling the JID-matching fix (above) depends on. The memory concern that motivated it in the first place is already handled by Baileys' *default* `shouldSyncHistoryMessage` (excludes only the expensive `FULL` sync type), so the fix was to delete the override, not tune it.
